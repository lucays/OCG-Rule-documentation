...
-  | 这类效果各步处理不在同1个时点进行，而是按照文本顺序先后进行。
   ...
   | 「`交织棉羊`_」的连接端是融合怪兽和X怪兽，①效果处理时，『●融合：』效果特殊召唤了「`终末之骑士`_」后，即使对方场上没有怪兽，『●X：』效果不适用的场合，处理完「`终末之骑士`_」的效果也错过时点不能发动。\ `23/3/18 <https://yugioh-wiki.net/index.php?%A1%D4%A5%AF%A5%ED%A5%B7%A1%BC%A5%D7%A1%D5#faq>`__
...
对于『A，B』和『A。那之后/再B』这样的文本，基本上和『全部○○』的处理一致：

-  | 如果『尽可能』在A部分的文本中，只要能适用1张，就当作会正常适用，可以发动这类效果。
...
-  | 如果『尽可能』在B部分的文本中，只要A部分的效果可以适用，就可以发动这类效果，不需要考虑B部分是否能正常适用。
   | 例：
   ...
   .. note:: 类似的记述『全部』的效果如「`升阶魔法-源数之力`_」，处理也是一样的，即使场上没有可以被这个效果无效的卡也能发动。
...
-  | 特别地，记述『以下效果适用』等含有多个『●』的文本，只要有1个『●』效果可以适用，这个效果就可以发动。
...
.. _`交织棉羊`: https://ygocdb.com/?search=交织棉羊
