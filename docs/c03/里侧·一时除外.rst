...
.. note::
   ...
   | 上述变成里侧表示后如果仍然是『○○召唤』的怪兽，再反转召唤被无效并破坏的场合，也当作是『○○召唤』的怪兽被破坏。详见\ :ref:`○○召唤无效`\ 。
   | 例：
...
-  | 『这张卡的①的效果·特殊召唤的这张卡』这样，自身效果特殊召唤的情报在变成里侧表示后丢失。
   | 被一时除外的场合，情报不丢失，但是如果是需要发动的效果，不能发动。
   | 例：
   ...
   | 自身①效果特殊召唤的「`延迟天使`_」「`临时保险三角龙装`_」，因「`月之书`_」等效果变成里侧表示后，再变成表侧表示的场合，那个②效果不能再发动；因「`亚空间物质传送装置`_」等效果被一时除外的场合，回到场上后②效果不能再发动。
...
| 被一时除外的场合，攻击力·守备力的变化会消失，其他情报难以判断。
| 例：
| 不用解放召唤的「`迷雾恶魔`_」被一时除外的场合，结束阶段仍然会发动效果。
...
| 不用解放召唤的「`战栗之凶皇-始祖恶魔`_」变成里侧表示或被一时除外的场合，②效果不适用，也不破坏。\ `13/6/5 <https://yugioh-wiki.net/index.php?%A1%D4%C0%EF%D8%CB%A4%CE%B6%A7%B9%C4%A1%DD%A5%B8%A5%A7%A5%CD%A5%B7%A5%B9%A1%A6%A5%C7%A1%BC%A5%E2%A5%F3%A1%D5#faq>`__
| 「`雷仙神`_」用①的方法召唤后，变成里侧表示或被一时除外，再被破坏的场合，都不能发动②效果。\ `17/7/28 <https://www.db.yugioh-card.com/yugiohdb/faq_search.action?ope=5&fid=20822&keyword=&tag=-1&request_locale=ja>`__ \ `17/7/28 <https://www.db.yugioh-card.com/yugiohdb/faq_search.action?ope=5&fid=20821&keyword=&tag=-1&request_locale=ja>`__
