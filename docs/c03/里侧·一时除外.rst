...
| 例：
| 「`破坏龙 甘多拉`_」在反转召唤的结束阶段之前变成里侧表示再因效果反转，结束阶段不会发动②效果。被一时除外再在结束阶段回到场上时也一样。
...
| 由于『反转召唤』的情报变成里侧或被一时除外后丢失，因此『这个回合反转召唤』也丢失。
| 例：
...
| 怪兽发动过效果，那个连锁块处理后，再变成里侧表示或者被一时除外的场合，不再当作『把效果发动过的怪兽』。
| 例：
...
-  | 怪兽发动效果时，连锁发动卡的效果把这只怪兽变成里侧表示的场合，仍然是『把效果发动过的怪兽』。
   | 例：
   | 「`宝龙星-神数负屃`_」的①效果发动时，连锁发动「`月之书`_」，盖放了这只「`宝龙星-神数负屃`_」的场合，这个效果也正常适用。『这个效果发动过的这张卡从场上离开的场合回到持有者卡组最下面』效果也正常适用。\ `17/3/24 <https://www.db.yugioh-card.com/yugiohdb/faq_search.action?ope=5&fid=15027&keyword=&tag=-1&request_locale=ja>`__
