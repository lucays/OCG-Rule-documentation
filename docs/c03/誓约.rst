...
-  | 「`DDD 双晓王 末法神`_」等『这张卡○○召唤成功的回合，○○』、和「`超重武者 法螺贝-E`_」等『这个方法特殊召唤\ **成功**\ 的回合，○○』
...
-  | 「`风来王 野风`_」等『这个方法特殊召唤过的回合，○○』
   | 是效果，但特殊召唤之际就适用，「`技能抽取`_」适用时不会被无效。
   | 后续行动已经进行过，也能特殊召唤。
   | 例：
   | 「`风来王 野风`_」的①效果这样，『这个方法特殊召唤过的回合』效果，不是在场上适用的效果，即使场上存在「`技能抽取`_」，这类效果也不会被无效。「`恐龙摔跤手·席拉腔骨龙`_」「`强袭黑羽-雾雨之苦无鸟`_」「`燃烧拳击手 练习拳手`_」的①效果等，以及「`强袭黑羽-骤雨之雷切刀鸟`_」的『「`黑羽`_」怪兽为素材作同调召唤的这张卡当作调整使用』效果、「`邪神官 契伦·沙巴`_」的『这个效果特殊召唤的这张卡当作调整使用』效果、「`邪神官 契伦·沙巴`_」的『这个回合，这张卡不能攻击』效果、「`恶魔将星`_」的『这个效果特殊召唤的这张卡当作调整使用』效果也都一样，不会被无效。\ `22/12/10 <https://www.db.yugioh-card.com/yugiohdb/faq_search.action?ope=5&fid=20371&keyword=&tag=-1&request_locale=ja>`__
