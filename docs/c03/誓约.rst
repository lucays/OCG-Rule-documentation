...
-  | 不是效果所以在发动时会立即适用。
   ...
   .. note::
      ...
      | 改变怪兽卡名·种族·属性·等级·攻击力·守备力等（不包含表示形式）的永续效果适用中，先适用『不是○○怪兽不能○○召唤』的状况，可以『○○召唤』的只有原本状态符合条件的怪兽。在场上出现的时点立即变得不符合条件，就这样留在场上。
      | 这类效果适用中，即使先『○○召唤』的怪兽的原本状态不符合条件，由于在场上出现的时点立即符合条件，之后仍然可以发动记述『不是○○怪兽不能○○召唤』的效果。
      | 例：
...
.. note::
   ...
   | 效果处理部分的括号也有可能不是效果。
   | 例：
   | 「`七皇转生`_」的①效果处理时，『（（把持有超量素材的怪兽除外的场合那些超量素材也全部除外））』让作为X素材的「`悲剧之死狱乡演员`_」被除外的场合，是被效果除外，可以发动①效果。
...
| 「`DD 俄耳托斯`_」「`机关傀儡-梦魇`_」「`DDD 双晓王 末法神`_」的这类记述在特殊召唤成功后适用，是\ **在场上适用的效果**\ 。适用这个效果的玩家是特殊召唤成功时的控制者。\ `23/2/18 <https://www.db.yugioh-card.com/yugiohdb/faq_search.action?ope=5&fid=17824&keyword=&tag=-1&request_locale=ja>`__
...
| 「`风来王 野风`_」「`超重武者 法螺贝-E`_」的这类记述虽然是效果，但不是在场上适用的效果，「`技能抽取`_」适用时不会被无效。
| 特殊召唤之际的时点还不会适用这类效果。
...
.. note::
   ...
   | 「`星圣神龙 托勒密星团M7`_」「`永远的淑女 贝阿特丽切`_」「`No.77 七罪蛛`_」「`灾厄之星 提·丰`_」等特殊召唤怪兽的召唤方法之后记述的『这个方法特殊召唤过的回合』不是效果，其他处理同上，没有区别。
   | 例：
