...
.. tip::
   ...
   | 卡名基本用来举例，因此搜卡名不一定有结果。
   | 由于基本不用问答方式介绍，搜索时应该直接搜关键词。例如，可以搜『连锁处理』，不要搜『:strike:`连锁怎么处理`』。
   | 部分字段原文是英文的不会用翻译的中文字段。例如，「E·HERO 天空侠」可以搜「天空侠」，不要搜「:strike:`元素英雄`」。
   | 一些英文简称也不会翻译。例如，S·X·P对应同调·XYZ·灵摆，除了卡名或效果文本记述外，不会使用后者，不要搜『:strike:`同调·XYZ·灵摆`』。
